...
for i, ds in enumerate(['test', 'AIBL', 'FHS', 'NACC']):
    ...
    hdl_crv['E'][ds] = plot_curve(curve='roc', **roc_info['E'][ds], ax=axes[ds],
                                  **{'color': 'C{}'.format(i), 'hatch': '//////', 'alpha': .4, 'line': '--',
                                     'title': title})
    hdl_crv['F'][ds] = plot_curve(curve='roc', **roc_info['F'][ds], ax=axes[ds],
...
fig.savefig('./apoe_roc.tif', dpi=300)
...
for i, ds in enumerate(['test', 'AIBL', 'FHS', 'NACC']):
    title = ds if ds != 'test' else "ADNI Test"
    hdl_crv['E'][ds] = plot_curve(curve='pr', **pr_info['E'][ds], ax=axes[ds],
                                  **{'color': 'C{}'.format(i), 'hatch': '//////', 'alpha': .4, 'line': '--',
                                     'title': title})
    hdl_crv['F'][ds] = plot_curve(curve='pr', **pr_info['F'][ds], ax=axes[ds],
                                  **{'color': 'C{}'.format(i), 'hatch': None, 'alpha': .2, 'line': '-', 'title': title})
plot_legend(axes=axes, crv_lgd_hdl=hdl_crv, crv_info=pr_info, neo_lgd_hdl=neo_info)
fig.savefig('./apoe_pr.tif', dpi=300)
